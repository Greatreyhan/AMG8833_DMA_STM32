...
static uint8_t MSB[1];
static uint8_t counter_dma = 0;
...
// Get Address
uint8_t amg8833_get_addr(void){
	return AMG8833_ADDR;
}

// First Call DMA
void amg8833_get_temp_DMA(void){
	HAL_I2C_Mem_Read_DMA(hi2c1, AMG8833_ADDR, AMG8833_T01L, 1, LSB, 1);
}

// Get Temperature DMA
amg8833_temperature_table_t amg8833_get_temp_DMA_handler(void){
	amg8833_temperature_table_t handler;
	if(counter_dma > 127) counter_dma = 0;
		if(counter_dma % 2 == 0){
			handler.pix[counter_dma/2] = (double) ((((MSB[0] & 0x07) << 8)| LSB[0]))*0.25;
			HAL_I2C_Mem_Read_DMA(hi2c1, AMG8833_ADDR, (AMG8833_T01L+counter_dma++), 1, LSB, 1);
			
		}
		else{
			HAL_I2C_Mem_Read_DMA(hi2c1, AMG8833_ADDR, (AMG8833_T01L+counter_dma++), 1, MSB, 1);
		}
	return handler;
}
